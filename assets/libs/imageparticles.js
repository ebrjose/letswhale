...
const ImageParticles = {
  ...
  initialize: function(
    ...
    imageData,
    densityPara = 14,
    produceDistancePara = 28,
    baseRadiusPara = 2,
    maxLineThicknessPara = 1,
    reactionSensitivityPara = 2,
    lineThicknessPara = 1
  ) {
      ...
      for (j = 0; j < this.particles.length; j++) {
        ...
        if (distance <= this.produceDistance) {
          this.context.lineWidth =
            (1 - distance / this.produceDistance) *
  ...
  pointerMove: function(event) {
    ImageParticles.mouse.x =
      event.offsetX || event.layerX - ImageParticles.canvas.offsetLeft
    ImageParticles.mouse.y =
      event.offsetY || event.layerY - ImageParticles.canvas.offsetTop
  },
